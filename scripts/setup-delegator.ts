/**
 * FlowVault — DeleGator Setup Script
 *
 * Two modes:
 *   1. --testnet   Use Base Sepolia (supported by toolkit) to get env values for dev
 *   2. --deploy    Deploy DeleGator contracts to Flow EVM (needs funded wallet)
 *
 * Usage:
...
import { flowMainnet, baseSepolia } from "viem/chains";
import {
...
const FLOW_RPC = process.env.FLOW_EVM_RPC_URL ?? "https://mainnet.evm.nodes.onflow.org";
...
if (!mode || (mode !== "--testnet" && mode !== "--deploy")) {
  ...
  console.log("  pnpm tsx scripts/setup-delegator.ts --deploy    # Prod: deploy to Flow EVM");
  process.exit(0);
}

async function runTestnet() {
  console.log("🔧  FlowVault — DeleGator Setup (Base Sepolia testnet)\n");
  console.log("ℹ️   Flow EVM is not yet supported by the toolkit.");
  ...
  console.log(`FLOW_SMART_ACCOUNT_ADDRESS=${smartAccount.address}   # ⚠️  Base Sepolia address — will differ on Flow`);
  ...
  console.log("\n⚠️   These addresses are for Base Sepolia. Run --deploy once you're ready for Flow EVM.");
}

async function runDeploy() {
  console.log("🚀  FlowVault — DeleGator Deployment to Celo Mainnet\n");
  console.log("⚠️   This will deploy multiple contracts and spend FLOW for gas.\n");
  ...
  const balanceFlow = Number(balance) / 1e18;
  console.log(`Balance: ${balanceFlow.toFixed(4)} FLOW\n`);

  if (balanceFlow < 0.1) {
    console.error("❌  Insufficient balance. Need at least 0.1 FLOW to deploy contracts.");
  ...
  console.log("Deploying DeleGator environment to Flow EVM... this may take a minute.\n");
  ...
  overrideDeployedEnvironment(flowMainnet.id, PREFERRED_VERSION, environment);
  ...
  console.log(`FLOW_SMART_ACCOUNT_ADDRESS=${smartAccount.address}`);
  console.log(`MM_DELEGATION_CONTRACT=${environment.DelegationManager}`);
